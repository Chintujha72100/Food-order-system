...
export default function Cart() {
    ...
    const applyCoupon = async () => {
        try {
            const { data } = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/coupons/validate`, { code: coupon });
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/orders`, {
                items: cart.map(item => ({
